from fastapi import FastAPI, APIRouter, HTTPException, Depends, Response, Request, status, File, UploadFile
...
# Include Approvals Hub Routes
try:
    from routes.approvals_hub_routes import router as approvals_hub_router
    api_router.include_router(approvals_hub_router)
    print("[Approvals Hub] Router mounted at /api/approvals-hub")
except Exception as exc:
...
async def update_vendor(vendor_id: str, vendor_update: Vendor, request: Request):
    ...
    audit_log = AuditLog(
        ...
        user_name=user.name,
        user_role=user.role,
        details=f"Vendor updated. Changes: {json.dumps(changes) if changes else 'No tracked changes'}"
    )
    audit_doc = audit_log.model_dump()
    audit_doc["timestamp"] = audit_doc["timestamp"].isoformat()
    await db.audit_logs.insert_one(audit_doc)
...
async def get_purchase_order(po_id: str, request: Request):
    """Get purchase order by ID - RBAC: requires viewer permission"""
    from utils.auth import require_permission
    from utils.permissions import Permission
...
async def get_osrs(request: Request):
    """Get all OSRs - RBAC: requires viewer permission with data filtering"""
    from utils.auth import require_permission
    from utils.permissions import Permission, should_filter_by_user
